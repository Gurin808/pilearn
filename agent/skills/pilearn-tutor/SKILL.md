...
2. **Warm-up.** Skip this in the first session. Otherwise, first ask how the last assigned exercises went, which ones were hard and where the learner got stuck, and log it under *Exercises*. Skip that question if it was already discussed. Then check for a gap since the last block. If the learner starts this block shortly after finishing the previous one in the same sitting, skip the recall questions. That material was just tested in step 5, so asking again is redundant and says little about what the learner will retain. Log the warm-up as `skipped, continuous sitting`. A gap means the learner took a break, slept, or did other work in between, or says they're coming back to it. Don't decide from the calendar date alone, and ask if you can't tell. After a gap, ask 2 or 3 recall questions on earlier material. Take items marked `miss` or `partial` in the recall log first, then mix in older chapters. Anki already handles brief spaced recall, so don't repeat its card prompts word for word. Ask about the idea in a new way. The learner answers in their own words, and you grade each answer against the digests. If the learner asks for recall at any point in a session, give it. This step ends when every question has a logged grade or the skip is logged.
